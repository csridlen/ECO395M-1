...
as_tibble(rf_spec$agr_profiles) %>%
  ggplot(aes(`_x_`, `_yhat_`)) + 
  geom_line(size = 1.2, alpha = 0.8) + 
  labs("Partial dependence of total cases on specific humidity")
...
```
The final plot shows the partial dependence of total cases on average DTR, or Average Diurnal Temperature Range for the week. It seems that all of these variables are significant in predicting total cases of dengue fever in a given week.
...
The baseline model for stepwise selection was `lm(medianHouseValue ~ . - longitude - latitude, data = ca_train).`
...
api <- "AIzaSyAkBLpObQN-MDl0opBQIMh-Tb0AiiFdFak"
register_google(api)
...
ggmap(ca_map) + 
  ...
  scale_color_viridis_c(option = "viridis", labels = c("0", "200000", "300000", "400000", "500000"))
```
...
Looking at the residuals from the preditions on the test set, the model appears to have done a good job at predicting (except for some over-predictions).