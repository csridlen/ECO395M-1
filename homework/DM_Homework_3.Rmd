...
x = sparse.model.matrix(revenue_sqft ~ . -CS_PropertyID - cluster, data = greenbuildings)
y = greenbuildings$revenue_sqft

xtrain = sparse.model.matrix(revenue_sqft ~ . -CS_PropertyID - cluster, data = green_train)
ytrain = green_train$revenue_sqft

xtest = sparse.model.matrix(revenue_sqft ~ . -CS_PropertyID - cluster, data = green_test)
...
lasso_coef <- predict(lasso_out, type = "coefficients", 
                      s = best_l)[1:20, ]
lasso_coef
```
