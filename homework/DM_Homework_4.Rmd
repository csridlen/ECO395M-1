...
Which items are the most frequent? The following table summarizes the 5 most popular items purchased.
...
inspect(head(sort(rules_soda, by = "lift"), 5))
```

Click on the dropdown in the left-hand corner to isolate either the rules associated with the item itself, or to isolate specific rules. Rules are shaded by the size of their lift; bright red rules have the highest lifts.
